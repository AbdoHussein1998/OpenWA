...
function makeRoot({ dashboard = false, patcher = false, previewPatcher = false, statusPatcher = false } = {}) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'openwa-postinstall-'));
  if (dashboard) fs.mkdirSync(path.join(root, 'dashboard'));
  if (patcher || previewPatcher || statusPatcher) {
...
test('planSteps: dashboard and all patchers run in stable order', () => {
  const steps = planSteps(makeRoot({ dashboard: true, patcher: true, previewPatcher: true, statusPatcher: true }));
