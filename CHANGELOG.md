...
- **Engine inventory drift guard** — `src/engine/engine-inventory-parity.spec.ts` checks the two hand-filled columns of `docs/29`: a symbol marked as used must appear in adapter code rather than only in a comment, an interface method named in a wired cell must be `supported` for that engine in the capability matrix, and an event marked consumed must have a listener behind it — so the inventory cannot drift away from the adapters unnoticed the way it did before.
