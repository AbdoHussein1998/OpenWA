...
- The metrics reference now lists every series `GET /api/metrics` emits. It introduced its table as "the complete set — nothing else is emitted" while naming 10 of 15, so the webhook-failure, session-reconnect, restricted-session and pacing-refusal counters an operator would alert on were absent from the document they alert from. A gate compares the table against the renderer, and the text now says which series are conditional, since omitting one is not the same as reporting zero.
...
- The release workflow's non-root check can now run at all. The job invoking `scripts/smoke-test-non-root.sh` had no `actions/checkout`, so the script was absent from the workspace and the step exited 127 — failing every tag build and blocking the only path that publishes `latest`, rather than guarding it. A gate now binds the precondition across every workflow: a job that runs a repo-relative command must check the repo out.
