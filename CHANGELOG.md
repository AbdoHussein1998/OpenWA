...
- `WEBHOOK_CONTACT_DETAILS` was undocumented outside `.env.example`; the event catalog now names the twelve fields it adds to `contact` on `message.received`, notes that opting in costs no extra WhatsApp lookup, and records that the flag is read on the whatsapp-web.js path only.
