...
describe('feature-flags', () => {
  describe('computeFeatureFlags', () => {
    ...
    it('treats opt-in flags (autoStart, resolveLid) as ON only for the exact string "true"', () => {
      ...
      for (const v of [
        ...
        expect(
          computeFeatureFlags({
            AUTO_START_SESSIONS: v,
          }).autoStartSessions,
        ).toBe(false);

        expect(
          computeFeatureFlags({
            RESOLVE_LID_TO_PHONE: v,
          }).resolveLidToPhone,
        ).toBe(false);
      }
    });
    ...
    it('parses simulateTypingMaxMs with Number()||5000 semantics (0/negative/non-numeric fall back)', () => {
      ...
      expect(
        computeFeatureFlags({
          SIMULATE_TYPING_MAX_MS: 'abc',
        }).simulateTypingMaxMs,
      ).toBe(5000);

      expect(
        computeFeatureFlags({
          SIMULATE_TYPING_MAX_MS: undefined,
        }).simulateTypingMaxMs,
      ).toBe(5000);
    });
  });
  ...
  describe('bundled production compose', () => {
    it('forwards AUTO_START_SESSIONS to the container', () => {
      ...
      expect(
        ...
        /^\s*- AUTO_START_SESSIONS=\$\{AUTO_START_SESSIONS:-\}$/m,
      );
    });
  });
  ...
  describe('bundled compose forwards the plugin redirect policy', () => {
    /*
     * Extract one service directly under `services:`.
     *
     * Normalize CRLF first because Windows Git checkouts may preserve CRLF.
     * A service header is exactly two spaces deep; nested keys are four spaces
     * deep, so the next two-space `name:` line ends the current service block.
     */
    function extractTopLevelService(
      compose: string,
      serviceName: string,
    ): string {
      const lines =
      ...
      for (
        ...
        /*
         * Leaving the `services:` mapping altogether also ends the block.
         * Top-level Compose keys are unindented.
         */
        if (
          /^[A-Za-z0-9_-]+:\s*$/.test(
            lines[i],
