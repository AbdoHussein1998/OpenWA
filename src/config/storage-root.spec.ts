...
describe('isStorageRootWritable', () => {
  ...
  it('reports an EXISTING but unwritable root as not writable (the #1065 case existsSync misses)', () => {
    /*
     * Native Windows/NTFS does not implement POSIX write permission bits
     * through chmod(0555), so this fixture cannot manufacture an unwritable
     * directory there. The implementation still uses accessSync(W_OK);
     * exercise the POSIX filesystem semantics where chmod can create the
     * condition this regression test needs.
     */
    if (
      process.platform ===
      'win32'
