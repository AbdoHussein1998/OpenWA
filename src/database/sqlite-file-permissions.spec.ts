...
} from './sqlite-file-permissions';

const modeOf = (path: string): number =>
  statSync(path).mode & 0o777;

const isWindows =
  process.platform === 'win32';

const expectChmod600 = (
  path: string,
): void => {
  expect(
    fs.chmodSync,
  ...
  /*
   * Native Windows/NTFS does not expose POSIX chmod mode semantics through
   * Node's stat().mode. Keep validating the chmod request itself everywhere,
   * and validate the resulting mode bits only on POSIX platforms.
   */
  if (!isWindows) {
    expect(
      modeOf(path),
...
describe('tightenSqliteFilePermissions', () => {
  ...
  afterEach(() => {
    chmodSync(
      dir,
      0o755,
    ); // a 0o500 test dir must still be cleanable
...
describe('SqlitePermissionsBoot', () => {
  ...
  it('falls back to the documented default paths when config supplies none', () => {
    ...
    // Must not throw even when neither default file exists
    // (e.g. a fresh checkout before first boot).
    expect(
      () =>
        boot.onApplicationBootstrap(),
    ).not.toThrow();
