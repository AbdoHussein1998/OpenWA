...
export class MessageController {
  ...
  @ApiBody({
    ...
    examples: SEND_TEXT_BODY_EXAMPLES,
  })
  @ApiResponse({
    status: 201,
    description:
  ...
  async sendDocument(
    ...
    return this.messageService.sendDocument(
      sessionId,
      dto,
    );
  }
  ...
  async sendContact(
    ...
    return this.messageService.sendContact(
      sessionId,
      dto,
    );
  }
  ...
  async sendSticker(
    ...
    return this.messageService.sendSticker(
      sessionId,
      dto,
    );
  }
  ...
  async reply(
    ...
    return this.messageService.reply(
      sessionId,
      dto,
    );
  }
  ...
  async forward(
    ...
    return this.messageService.forward(
      sessionId,
      dto,
    );
  }
  ...
  @ApiResponse({
    status: 200,
    description:
      'Reaction added or removed. Send empty emoji to remove reaction.',
    type: MessageActionResponseDto,
  })
  @ApiResponse({
    status: 400,
    description:
      'Session not active or message not found',
  })
  @ApiResponse({
    status: 409,
    description:
  ...
  // ===========================================================================
  // Live chat history / reactions / media reads
  // ===========================================================================
  ...
  @ApiParam({
    name: 'chatId',
    description:
      'Chat ID (e.g. 1234567890@c.us or groupId@g.us)',
  })
  @ApiQuery({
    name: 'limit',
    required: false,
  ...
  @ApiQuery({
    ...
    type: Boolean,
    description:
      'When true, downloads media (base64) for messages that have it. Slower; default false.',
  })
  @ApiQuery({
    name: 'deep',
    required: false,
  ...
  @ApiResponse({
    status: 200,
    description:
      'List of reactions with senders',
    type: [
      MessageReactionDto,
    ],
  })
  ...
  async getReactions(
    ...
    return this.messageService.getMessageReactions(
      sessionId,
      chatId,
      messageId,
    );
  ...
  @ApiParam({
    name: 'messageId',
    description:
      'WhatsApp message ID whose media to download',
  })
  @ApiResponse({
    status: 200,
    description:
  ...
  async getChatMedia(
    ...
    const {
      ...
      await this.messageService.getChatMedia(
        sessionId,
        chatId,
        messageId,
      );
  ...
  @ApiResponse({
    status: 400,
    description:
      'Session not active, or the target message is not a poll',
  })
  @ApiResponse({
    status: 404,
    description:
  ...
  @ApiResponse({
    status: 501,
    description:
      'Not supported on the Baileys engine',
  })
  @ApiResponse({
    status: 409,
    description:
  ...
  async votePoll(
    ...
    return this.messageService.votePoll(
      sessionId,
      dto,
    );
  }
  ...
  @ApiResponse({
    status: 400,
    description:
      'Session not active, or durationSeconds is not one of 86400 / 604800 / 2592000',
  })
  @ApiResponse({
    status: 403,
    description:
  ...
  async starMessage(
    ...
    return this.messageService.starMessage(
      sessionId,
      dto,
    );
  }
  ...
  @ApiResponse({
    status: 202,
    description:
      'Batch created and processing started',
  ...
  async sendBulk(
    ...
    const batch =
      await this.bulkMessageService.createBatch(
        sessionId,
        dto,
      );
    ...
    return {
      ...
      statusUrl:
        `/api/sessions/${sessionId}/messages/batch/${batch.batchId}`,
    };
  }
  ...
  async getBatchStatus(
    ...
    const batch =
      await this.bulkMessageService.getBatchStatus(
        sessionId,
        batchId,
      );
  ...
  @ApiResponse({
    status: 400,
    description:
      'Batch already completed, cancelled, or failed (terminal statuses are exclusive)',
  })
  @ApiResponse({
    status: 404,
    description:
  ...
  async cancelBatch(
    ...
    const batch =
      await this.bulkMessageService.cancelBatch(
        sessionId,
        batchId,
      );
