...
describe('SessionService', () => {
  ...
  beforeEach(async () => {
    ...
    dataSource = {
      transaction: jest.fn().mockImplementation(async (cb: (manager: unknown) => Promise<unknown>) => {
        const manager = {
          create: jest.fn().mockImplementation((_target: unknown, entity: unknown) => entity),
          find: jest.fn().mockResolvedValue([]),
          save: jest.fn().mockImplementation((...args: unknown[]) => Promise.resolve(args.at(-1))),
  ...
  describe('delete', () => {
    ...
    it('preserves message/batch history, snapshots the tombstone, and removes only Session-owned runtime/config rows', async () => {
      const session = createMockSession({ ownerTeamLeaderId: 'tl-1' });
      (repository.findOne as jest.Mock).mockResolvedValue(session);
      ...
      const pendingBatch = {
        ...
        status: 'processing',
        messages: [
          { chatId: 'c@c.us', type: 'image', content: { image: { base64: 'AAAA', mimetype: 'image/png' } } },
        ],
        progress: { total: 2, sent: 1, failed: 0, pending: 1, cancelled: 0 },
        completedAt: null,
      } as unknown as MessageBatch;

      const managerCreate = jest.fn().mockImplementation((_target: unknown, entity: unknown) => entity);
      const managerFind = jest.fn().mockResolvedValue([pendingBatch]);
      const managerSave = jest.fn().mockImplementation((...args: unknown[]) => Promise.resolve(args.at(-1)));
      ...
      expect(managerCreate).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          sessionId: 'sess-uuid-1',
          name: 'test-session',
          ownerTeamLeaderId: 'tl-1',
        }),
      );
      expect(managerSave).toHaveBeenCalledWith(SessionTombstone, expect.objectContaining({ sessionId: 'sess-uuid-1' }));

      expect(managerFind).toHaveBeenCalledWith(
        MessageBatch,
        expect.objectContaining({
          where: expect.objectContaining({ sessionId: 'sess-uuid-1' }),
        }),
      );
      expect(pendingBatch.status).toBe('cancelled');
      expect(pendingBatch.progress.pending).toBe(0);
      expect(pendingBatch.progress.cancelled).toBe(1);
      expect(pendingBatch.completedAt).toBeInstanceOf(Date);
      expect(JSON.stringify(pendingBatch.messages)).not.toContain('AAAA');
      expect(managerSave).toHaveBeenCalledWith(pendingBatch);

      // Message and MessageBatch are durable history: neither table is deleted during Session removal.
      expect(managerDelete).not.toHaveBeenCalledWith(Message, expect.anything());
      expect(managerDelete).not.toHaveBeenCalledWith(MessageBatch, expect.anything());
