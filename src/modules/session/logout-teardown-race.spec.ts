...
async function expectNameTeardownPending(p: Promise<unknown>): Promise<void> {
  ...
  // Nest serializes an object-response ConflictException into
  // { statusCode, message, error, ...fields }. The stable code is attached on the
  // response object so REST callers can branch on it without parsing the human message.
...
describe('SessionService logout() name-scoped teardown fence', () => {
  ...
  beforeEach(async () => {
    ...
    // SessionService now owns the cross-database cleanup that clears
    // Agent.assignedSessionId after a Session is successfully deleted.
    //
    // Agent lives in the `main` database, so this must use the exact
    // getRepositoryToken(Agent, 'main') token used by production SessionModule.
      ...
      providers: [
        ...
        },
        {
          ...
          useValue: {
            get: jest
              .fn()
              .mockImplementation(<T>(_k: string, d?: T): T => d as T),
          },
        },
        {
          provide: LidMappingStoreService,
          useValue: {
            remember: jest.fn(),
          },
        },
        {
          provide: StatusStoreService,
          useValue: {
            ingest: jest.fn(),
          },
        },
  ...
  const enginesOf = () =>
    (service as unknown as { engines: Map<string, unknown> }).engines;
  ...
  const lastStatusOf = () =>
    (
      lifecycle as unknown as {
        lastDispatchedStatus: Map<string, unknown>;
      }
  ...
  it('quick-settling teardown: start waits for it, then proceeds', async () => {
    ...
    const engine = {
      logout: jest.fn().mockReturnValue(wedgedLogout),
    };
    ...
    try {
      ...
      // logout loses its deadline race → 502
      await expect(logoutCall).rejects.toBeInstanceOf(
        BadGatewayException,
      );
  ...
  it('quick-settling teardown: delete waits for it, then proceeds and purges', async () => {
    ...
    try {
      ...
      expect(
        engineFactory.purgeSessionData,
      ...
      expect(
        engineFactory.purgeSessionData,
      ).toHaveBeenCalledWith(SESSION_NAME);
  ...
  it('old-name teardown still pending past 10s: start rejects 409 and does not create the engine', async () => {
    ...
    const engine = {
      logout: jest.fn().mockReturnValue(neverSettles),
    };
    ...
    try {
      ...
      // logout loses its race → 502; raw keeps running
      await jest.advanceTimersByTimeAsync(10_000);
      ...
      // Mark rejection handled across the timer advance below.
      startCall.catch(() => undefined);

      // Bounded wait exhausts → fail CLOSED.
      await jest.advanceTimersByTimeAsync(10_000);
  ...
  it('old-name teardown still pending past 10s: delete rejects 409, transaction parent removal and purgeSessionData not called (unique name still reserved)', async () => {
    ...
    // delete() runs on the lifecycle owner now, so the swap must land
    // there to intercept the path under test.
    (
      lifecycle as unknown as {
        dataSource: unknown;
      }
    ...
    try {
      ...
      // Mark rejection handled across the timer advance below.
      deleteCall.catch(() => undefined);

      // First fence already pending → fail fast.
      await jest.advanceTimersByTimeAsync(10_000);
  ...
  it('on a first-fence 409, start does not touch reconnect timer / engine / last status / error / recovery state', async () => {
    // Arm a reconnect timer + a stale FAILED error + a last-dispatched status BEFORE the start
    // attempt, then ensure a refused start (409) leaves ALL of them untouched.
    const neverSettles = new Promise<void>(() => undefined);
    ...
    try {
      ...
      const enginePresentBefore =
        enginesOf().has(SESSION_UUID);
      ...
      expect(
        reconnectStatesOf().has(SESSION_UUID),
      ).toBe(true);

      expect(
        errorsOf().get(SESSION_UUID),
      ).toBe('previously failed');

      expect(
        lastStatusOf().get(SESSION_UUID),
      ).toBe(SessionStatus.FAILED);

      // The engine map was not mutated by the refused start either.
      expect(
        enginesOf().has(SESSION_UUID),
      ).toBe(enginePresentBefore);
    } finally {
      jest.useRealTimers();
    }
  });
  ...
  it('a refused delete does NOT drop the pending fence', async () => {
    ...
    try {
      ...
      // First fence exhausts its bounded wait → fail CLOSED.
      await jest.advanceTimersByTimeAsync(10_000);
  ...
  it('a logout starting after delete first fence but before engine eviction is caught by the second fence', async () => {
    // delete() runs two fences: one right after findOne, one right after the engine is evicted.
    // A logout that slips past fence #1 (no pending teardown yet) but captures the live engine
    // before eviction must register its destructive promise synchronously
    // (the adapter fires onCredentialTeardownStarted the instant engine.logout() begins),
    // so fence #2 sees it and the delete refuses instead of freeing the name while an old
    // remover is live.
    (repository.findOne as jest.Mock).mockResolvedValue(
      createMockSession({
        id: SESSION_UUID,
    ...
    // The engine is torn down via forceDestroy during delete. A concurrent logout that captured
    // this engine fires its credential-teardown callback synchronously inside that teardown
    // window — between fence #1 and fence #2. Drive the service's own tracking the same way
    // the adapter would.
    const trackPending = (
      lifecycle as unknown as {
        trackPendingCredentialTeardown: (
          name: string,
          raw: Promise<void>,
        ) => void;
      }
    ...
    try {
      ...
      // Second fence exhausts its bounded wait.
      await jest.advanceTimersByTimeAsync(10_000);
      ...
      // The engine was already destroyed and evicted before this fence, so the surviving row must
      // not keep advertising a status it can no longer honour — the retry this 409 asks for should
      // not have to look past a session that still reads READY with nothing behind it.
      expect(
        enginesOf().has(SESSION_UUID),
      ).toBe(false);
  ...
  it('two concurrent logouts for the same name keep the fence until BOTH settle', async () => {
    ...
    // (still-running) raw settles — otherwise the second's fast settlement would drop the entry
    // while the first rm is still pending.
    ...
    const logout = jest
      ...
      .mockImplementationOnce(() =>
        // The client is already tearing down after the first logout captured it; mirror the
        // adapter's own "no live client" refusal. Built lazily so no stray unhandled rejection
        // is created.
        Promise.reject(
          new Error(
            'No live WhatsApp Web client — the unlink was not sent',
          ),
        ),
      );
    ...
    try {
      ...
      // Mark rejection handled across the timer advances below.
      ...
      // Still wedged → fence STILL up
      // (the fast settlement of the second did not drop it).
      expect(pendingOf().has(SESSION_NAME)).toBe(true);
  ...
  it('map assertions use the session name, not the UUID', async () => {
    // Regression guard: the key MUST be the session name
    // (the on-disk auth-dir key), never the UUID.
    const engine = {
