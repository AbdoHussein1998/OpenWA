...
describe('PluginInstanceService', () => {
  ...
  it('allows a concrete scope whose Session is temporarily absent (scope is configuration, not an FK)', async () => {
    const inst = await service.create('chatwoot', 'missing-session', { sessionScope: 'gone-sess' });
    expect(inst.sessionScope).toBe('gone-sess');
  });
...
describe('PluginInstanceService provisioning', () => {
  ...
  it('normalizes empty scope to null but preserves the explicit "*" wildcard', async () => {
    const minted = await service.mint('chatwoot', 'm1', { sessionScope: '' });
    expect(minted.sessionScope).toBeNull();

    const wildcard = await service.mint('chatwoot', 'm2', { sessionScope: '*' });
    expect(wildcard.sessionScope).toBe('*');
    ...
    expect(patched?.sessionScope).toBe('*');
  });
});
