...
describe('PluginInstanceService', () => {
  ...
  beforeEach(async () => {
    ds = new DataSource({ type: 'better-sqlite3', database: ':memory:', entities: [PluginInstance, Session], migrations: [] });
    await ds.initialize();
...
describe('PluginInstanceService provisioning', () => {
  ...
  it('normalizes empty and legacy wildcard sessionScope to null, never a literal empty/wildcard value', async () => {
    // NULL is the canonical persisted unscoped/inherited form and is required for a real nullable FK.
    ...
    expect(wildcard.sessionScope).toBeNull();
