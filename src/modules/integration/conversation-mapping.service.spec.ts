...
describe('ConversationMappingService', () => {
  ...
  beforeEach(async () => {
    ds = new DataSource({
      type: 'better-sqlite3',
      database: ':memory:',
      // Relation targets must be registered now that the OLTP mapping has real TypeORM relations.
      entities: [ConversationMapping, PluginInstance, Session],
      migrations: [],
    });
    await ds.initialize();
  ...
  it('rebindSession moves an existing row onto another session (forward key moves with it)', async () => {
    await service.upsert(key, 'conv-1');
    const row = await service.get(key);
    if (!row) throw new Error('expected a mapping row');
