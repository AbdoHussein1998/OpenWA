...
// Covers every contactTools handler via the real invokeTool path:
// credential validation -> role/capability authorization -> tenant authorization -> Zod -> handler.
...
describe('contactTools', () => {
  it('ContactFindAll delegates to getContacts with paging', async () => {
    ...
    const out = await run(
      makeTools({
        getContacts,
      } as unknown as ContactService).get(
        'ContactFindAll',
      )!,
      {
        sessionId: 's1',
  ...
  it('ContactResolvePhone delegates to resolveContactPhone', async () => {
    const resolveContactPhone = jest
      .fn()
      .mockResolvedValue(
        '628123',
      );
